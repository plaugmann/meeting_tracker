  ...
  try {
    ...
    if (type === 'user') {
      ...
      Array.from(userMap.values())
        ...
        .forEach((data) => {
          csv += `"${data.userName || 'No name'}","${data.userEmail}",${data.count}\n`;
        });
      ...
      Array.from(customerMap.values())
        .sort((a, b) => b.count - a.count)
        .forEach((data) => {
          csv += `"${data.customerName}",${data.count}\n`;
        });
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="meetings-report-${type}-${Date.now()}.csv"`,
      },
    });
  } catch (error) {
